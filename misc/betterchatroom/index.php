...
function chatroomForm() {
    ...
    <div id="chatroomform"> 
        ...
        <p>public room is at bookmarklets.rf.gd/talkroom</p>
    </div>';
}

// Custom chatroom functionality
function createOrJoinChatroom() {
    if (isset($_POST['create_chatroom'])) {
        if ($_POST['chatroom_name'] != "" && $_POST['password'] != "") {
            ...
            if (preg_match('/^[a-zA-Z0-9_]+$/', $chatroom_name) && preg_match('/^[a-zA-Z0-9_]+$/', $password)) {
                // Chatroom name and password are valid, proceed with creating/joining the chatroom
                $_SESSION['chatroom_name'] = $chatroom_name;
                $_SESSION['password'] = $password;
                
                // Save the entered chatroom details in session
                echo '<script>window.location.href = "talkroom.php";</script>';
            } else {
                echo '<span class="error">Please enter a valid chatroom name and password</span>';
            }
        } else {
